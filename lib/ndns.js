...
class Message extends EventEmitter {
    ...
    sendTo(socket, port, host) {
        ...
        if ((n = this.writeOnce(_maxmsg, _maxmsg.length)) !== -1) {
            ...
            // TODO I think this tmp is unnecessary, could just be
            // socket.send(_maxmsg, 0, n, port, host, cb)
...
class Server extends EventEmitter {
    ...
    constructor(type, requestListener) {
        ...
        this.socket.on("message", (msg, rinfo) => {
            ...
            debug(hexdump(msg, msg.length, 16));
...
class Client extends EventEmitter {
    ...
    request(port, host) {
        const req = new ClientRequest(this, {address: host, port});
        return req;
    }
}
